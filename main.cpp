...
int main() {
    ...
    std::wcout << "***Created by Michael***" << std::endl << std::endl;
    std::wcout << "使用提示：" << std::endl;

    //这行输出很奇妙，不好评价，导致我去找半天getline的BUG;
    std::wcout << "1、请输入文件夹路径例如:\"D:\\Folder\"" << std::endl;
    std::wcout << "2、根目录请以\"\\\"结尾" << std::endl;
            ...
            try {
                ...
                if (std::filesystem::is_regular_file(*dir)) {
                    ...
                    std::cout << "FileName:" << fileInfo.fileName << "    "
                              ...
                              << fileInfo.hash << std::endl << fileInfo.path << std::endl;
                }
                ++dir;
            } catch (const std::filesystem::filesystem_error &e) {
